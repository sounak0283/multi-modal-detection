...
from perimeter.boundary.geometry import clamp_truncated_feet
from perimeter.detect.yolox_onnx import Detections

log = logging.getLogger("perimeter.track")
...
class TrackedDetections:
    ...
    def foot_points(self, frame_height: int | None = None) -> np.ndarray:
        ...
        With `frame_height`, boxes cut off by the bottom of the frame have their foot
        point pulled into the frame's bottom band - see `geometry.clamp_truncated_feet`.
        """
        if len(self) == 0:
            return np.zeros((0, 2), np.float32)
        xs = (self.xyxy[:, 0] + self.xyxy[:, 2]) / 2.0
        points = np.stack([xs, self.xyxy[:, 3]], axis=1).astype(np.float32)
        if frame_height is not None:
            points = clamp_truncated_feet(points, frame_height)
        return points
